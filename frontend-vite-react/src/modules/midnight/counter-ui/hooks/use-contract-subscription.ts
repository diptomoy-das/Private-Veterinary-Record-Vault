...
export const useContractSubscription = (
  ...
  useEffect(() => {
    ...
    const subscription = contractStates.observable.subscribe(
  ...
  useEffect(() => {
    ...
    if (contractDeployment.status === "failed") {
      setErrorMessage(
        contractDeployment.error.message.length
          ? contractDeployment.error.message
    ...
    setIsLoading,
    setErrorMessage,
    setDeployedContractAPI,
  ]);

  useEffect(() => {
    if (deployedContractAPI) {
      const subscriptionDerivedState =
        deployedContractAPI.state$.subscribe(setContractState);
      return () => {
