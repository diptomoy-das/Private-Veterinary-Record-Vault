...
import { Counter, type CounterPrivateState } from '@eddalabs/vet-contract';
...
const counterCompiledContract = CompiledContract.make('counter', Counter.Contract).pipe(
  CompiledContract.withWitnesses({
    get_private_record: (context: any, pet_id: bigint) => {
      // Return [updatedPrivateState, witnessResult]
      // The witness returns a dummy Uint<248> value (31-byte bigint)
...
export const getCounterLedgerState = async (
  ...
  const state = await providers.publicDataProvider
    ...
    .then((contractState) => (contractState != null ? (Counter.ledger(contractState.data) as any).round ?? 0n : null));
...
export const deploy = async (
  providers: CounterProviders,
  // Use an empty object as initial state for the Vet Vault
  privateState: any = {}, 
): Promise<DeployedCounterContract> => {
...
export const increment = async (counterContract: DeployedCounterContract): Promise<FinalizedTxData> => {
  ...
  const finalizedTxData = await (counterContract as any).callTx.increment();  
